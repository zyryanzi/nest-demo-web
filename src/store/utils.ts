...
import { DashboardStateType } from './modules/dashboard/state'
...
// module 下的 key
type CommitNameType = AppStateType & DashboardStateType & UserStateType
 ...
 * @type {T} T  要派发actions的模块的类型
 * @example 使用方法如下  const result = await dispatchAction<UserActionsType>('dashboard','refreshToken',1)
 */
 ...
 * @example 使用方法如下  const result =  getStoreGetter<DashboardGetterType>('dashboard','list')
 */
export function getStoreGetter<T>(
    module: ModuleNameType,
    key: keyof T,
    ) {
