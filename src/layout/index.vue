...
const Layout = defineComponent({
    ...
    setup: (props, context) => {
        const handleClickOutside = () => {
            context.$store.dispatch('app/closeSideBar', {
                withoutAnimation: false
            })
        }
        const {sidebar} = computed((state: any) => {
            return 
        })
        ...
        onMounted(() => {
            if (Cookies.get('theme')) {
                context.$refs.theme.theme = Cookies.get('theme')
                context.$store.dispatch('settings/changeSetting', {
                    key: 'theme',
